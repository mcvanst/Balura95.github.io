// Prüfe, ob der Access Token vorhanden ist – sonst zurück zur Login-Seite
document.addEventListener('DOMContentLoaded', () => {
  if (!localStorage.getItem('access_token')) {
    window.location.href = 'index.html';
    return;
  }
  // Bei Touch-Ereignissen AudioContext aktivieren (iOS)
...
// Lade Tracks der Playlist per Spotify API
async function fetchPlaylistTracks(playlistId) {
  ...
  try {
    ...
    return data.items; // Array von Items; jedes Item hat ein "track"-Objekt
  } catch (error) {
...
// Initialisiere den Spotify Web Playback SDK für die mobile Version
...
window.onSpotifyWebPlaybackSDKReady = () => {
  ...
  window.mobilePlayer = player; // Speichere den Player global
  player.addListener('ready', ({ device_id }) => {
    window.deviceId = device_id;
  });
  
  // Fehler-Listener (optional: hier kannst du Fallback-Mechanismen einbauen)
  ...
  // Definiere window.playTrack für die mobile Version
...
// Funktion, die einen zufälligen Track aus der angegebenen Playlist abspielt
